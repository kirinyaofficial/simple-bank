...
import (
	...
	mock_db "github.com/kirinyaofficial/simple-bank/db/mock"
	db "github.com/kirinyaofficial/simple-bank/db/sqlc"
	"github.com/kirinyaofficial/simple-bank/token"
...
func TestTransferAPI(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			buildStubs: func(store *mock_db.MockStore) {
				store.EXPECT().GetAccount(gomock.Any(), gomock.Any()).Times(1).Return(db.Account{}, pgx.ErrTxClosed)
				store.EXPECT().TransferTx(gomock.Any(), gomock.Any()).Times(0)
			},
			checkResponse: func(recorder *httptest.ResponseRecorder) {
